...
2. **修改配置**：随时调整现有节点的入站协议、监听端口、认证密钥、AnyTLS 混淆规则（Padding Scheme，支持多行整段粘贴与自动补全 stop=*）、伪装域名、出口路由或流量策略；
3. **调整出口**：为指定节点单独切换主出口转发协议或恢复直连，变更后自动联动备用出口状态；
4. **备用出口**：全流程可视化管理备用出口与路线。可直观查看当前出口链路，支持添加新备用出口、删除指定备用出口或一键清空全部备用出口；变更后自动更新 sing-box 选择器并同步启停后台健康检测监控服务；
5. **调整流量**：单独调整指定节点的计费模式（单向/双向）、月度配额与每月重置日；
6. **删除节点**：删除指定节点并自动清理其关联的 nftables 流量监控规则与定时重置任务；
7. **分享链接**：一键生成各节点的客户端连接链接（如 `anytls://`, `ss://`, `trojan://`, `hy2://`, `vless://`, `socks5://`, `http://`）以及 sing-box 客户端 `outbounds` 配置段落。
