...
- **精准流量监控与配额管理**：
  ...
  - 支持按节点监听端口设置实时上传/下载限速（基于 Linux `tc`，纯数字默认按 Mbps 处理，如 `500` 等于 `500Mbps`）；
...
- **系统时钟守护（NTP 自动对时）**：安装与更新时自动守护系统时间同步服务（systemd-timesyncd / chrony），服务依赖等待时间同步完成（`time-sync.target`），从根源杜绝 Shadowsocks 2022、TLS 证书等因服务器开机时间漂移/倒流导致的握手失败。
...
1. **新增节点**：
   ...
   - 配置节点主出口（Direct，或转至外部 AnyTLS / Shadowsocks / Trojan / Hysteria2 / VLESS REALITY / SOCKS5 / HTTP 出口）；配置完成后可二选一决定是否添加备用出口（默认回车跳过）。
2. **修改配置**：随时调整现有节点的入站协议、监听端口、认证密钥、AnyTLS 混淆规则（Padding Scheme，支持多行整段粘贴与自动补全 stop=*）、伪装域名、网络出口路由或流量策略；
3. **网络出口**：全流程可视化管理节点的出口路线与优先级顺位列表。
   - 模糊化主备出口概念，按顺位（首选、备选2、备选3...）清晰呈现链路；
   - 支持自由添加出口、修改指定出口、删除指定出口；
   - 支持调整出口顺序（若配置了 Direct 直连出口，系统强制保障其永远处于末尾保底顺位且不支持调整顺序）；
   - 支持一键清空后置出口（仅保留第 1 首选出口）；变更后自动更新 sing-box 选择器并同步启停后台健康检测监控服务；
4. **调整流量**：单独调整指定节点的计费模式（单向/双向）、月度配额与每月重置日；
5. **删除节点**：删除指定节点并自动清理其关联的 nftables 流量监控规则与定时重置任务；
6. **分享链接**：一键生成各节点的客户端连接链接（如 `anytls://`, `ss://`, `trojan://`, `hy2://`, `vless://`, `socks5://`, `http://`）以及 sing-box 客户端 `outbounds` 配置段落。

### AnyTLS 混淆策略 (Padding Scheme) 独立配置文件

- **仓库配置文件**：项目 [`conf/anytls_padding.conf`](conf/anytls_padding.conf) 提供独立的规则配置文件。无需修改脚本代码，只需在此文件中调整各轮包长区间或 `stop=*`，提交后 VPS 执行 `sudo sbox update` 会自动检测变动、更新本地规则并平滑重载 sing-box 服务。
- **VPS 本地自由编辑**：配置文件在 VPS 上安装于 `/etc/sbox/anytls_padding.conf`，你也可以直接用 `nano /etc/sbox/anytls_padding.conf` 进行本地调整，保存后脚本检测到内容变动会自动重新渲染配置并重载服务。
- **单节点个性化隔离**：如果在节点管理菜单中为某个特定节点单独设置了自定义规则（例如美西专用规则），该节点将永久锁定自身专属规则，**绝不会被全局配置文件变动所影响**。

## 流量监控与重置日管理

进入 **4. 流量管理**：
- **实时流量列表**：查看所有节点的入站、出站、总流量、月配额、已使用百分比、重置日期与运行状态（正常 / 已超额阻断）；
- **端口实时限速**：流量管理页面的节点表格直接显示上行、下行限速；在“配置端口限速”中可分别调整客户端上传（入站）和客户端下载（出站）上限。输入 `0` 或留空表示不限速；
- **配置月配额与重置日**：
...
sudo sbox outbound                # 节点网络出口管理 (调整出口及优先级顺位)
...
- 出口健康监控脚本：`/etc/sbox/outbound_health_monitor.py`
- 出口健康监控运行状态：`/run/sbox/outbound-health.json`
- 流量重置历史审计日志：`/etc/sbox/traffic_reset.log`
- 配置备份归档目录：`/etc/sbox/backups/`
- 系统服务单元：`/etc/systemd/system/sing-box.service` 与 `/etc/systemd/system/sbox-outbound-monitor.service`（Systemd），或对应的 `/etc/init.d/` 服务（OpenRC）
